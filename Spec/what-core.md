...
`public protocol InteractionConfigParameter: Sendable`. Single required method: `func apply(to request: inout InteractionRequest)`. Conforming types write their value into the request; `MaxToolCalls` intentionally does nothing in `apply` and is consumed at a higher level.

## RetryPolicy
`public struct RetryPolicy: Sendable`. Configures automatic retry with exponential backoff. Properties: `maxAttempts: Int` (default: 3), `initialBackoff: Duration` (default: `.seconds(2)`), `backoffMultiplier: Double` (default: 2.0), `initialTimeout: Duration` (default: `.seconds(120)`), `timeoutMultiplier: Double` (default: 1.5), `maxTimeout: Duration` (default: `.seconds(300)`), `retryableStatusCodes: Set<Int>` (default: `[429, 500, 503]`), `onRetry: (@Sendable (RetryEvent) -> Void)?` (default: nil). Public memberwise init with all defaults.

## RetryEvent
`public struct RetryEvent: Sendable`. Describes a retry attempt. Properties: `attempt: Int` (1-indexed attempt that just failed), `maxAttempts: Int`, `error: GeminiInteractionsError`, `backoffDuration: Duration`, `nextTimeout: Duration`. Public memberwise init.

## Config parameter structs (16 total)
Each is `public struct X: InteractionConfigParameter` with a single public or private stored value and a public `init`. Types: `Temperature(Double)` (validates 0.0–2.0, sets `generationConfig.temperature`), `TopP(Double)` (validates 0.0–1.0, sets `generationConfig.topP`), `MaxOutputTokens(Int)` (validates > 0, sets `generationConfig.maxOutputTokens`), `Seed(Int)` (sets `generationConfig.seed`), `SystemInstruction(String)` (non-empty guard, sets `systemInstruction`), `PreviousInteractionId(String)` (non-empty guard, sets `previousInteractionId`), `Store(Bool)` (sets `store`), `Background(Bool)` (sets `background`), `ServiceTierParam(ServiceTier)` (sets `serviceTier`), `ThinkingLevelParam(ThinkingLevel)` (sets `generationConfig.thinkingLevel`), `ThinkingSummariesParam(ThinkingSummaries)` (sets `generationConfig.thinkingSummaries`), `ResponseFormatParam(ResponseFormat)` (sets `responseFormat`), `ResponseModalitiesParam([ResponseModality])` (non-empty guard, sets `responseModalities`), `MaxToolCalls(Int)` (`apply` is a no-op; `value` is `public let` so callers can read it), `EnvironmentParam(EnvironmentConfig)` (sets `environment`), `WebhookConfigParam(WebhookConfig)` (sets `webhookConfig`).

Note: `RequestTimeout` has been removed. Its functionality is subsumed by `RetryPolicy.initialTimeout`.
...
## InteractionsClient
`public actor InteractionsClient`. The primary HTTP client. Public init: `init(apiKey: String, apiRevision: String = "2026-05-20", retryPolicy: RetryPolicy? = RetryPolicy())` — uses `URLSession.shared`. Internal init: `init(apiKey:apiRevision:retryPolicy:session:)` — not public, used in tests with `MockURLProtocol`. The `retryPolicy` parameter defaults to `RetryPolicy()` (retry enabled with defaults); pass `nil` to disable retry. Base URL: `https://generativelanguage.googleapis.com`. Interaction requests go to `v1beta/interactions`; file search store requests go to `v1beta/fileSearchStores` (see `what-filesearchstores.md`). Every request includes headers `x-goog-api-key` and `Api-Revision`; `Content-Type: application/json` is added only when a request body is present. Internal helpers `baseURL`, `headers()`, `execute(_:)`, `decode(_:from:)`, `encode(_:)`, `makeRequest(url:method:body:)`, and `executeReturningResponse(_:)` are used by cross-file extensions (`FileSearchStores.swift`). Public methods: `send(_ request: InteractionRequest) async throws -> Interaction`, `get(id: String) async throws -> Interaction`, `delete(id: String) async throws`, `cancel(id: String) async throws`, `poll(id:timeout:interval:) async throws -> Interaction`, `nonisolated func stream(_ request: InteractionRequest) -> AsyncThrowingStream<InteractionStreamEvent, Error>`, `nonisolated func resumeStream(id:lastEventId:) -> AsyncThrowingStream<InteractionStreamEvent, Error>`. File search store methods documented in `what-filesearchstores.md`.
...
`public enum InteractionStreamEvent: Codable, Sendable`. Cases: `interactionCreated(Interaction)`, `interactionStatusUpdate(InteractionStatus)`, `stepStart(stepType: String, index: Int, name: String?)`, `stepDelta(InteractionStreamDelta, stepIndex: Int)`, `stepStop(index: Int)`, `interactionCompleted(Interaction)`, `error(String)`, `unknown`. Decoded via `event_type` key. `encode(to:)` is a no-op (events are receive-only). Unknown event types decode to `.unknown` and are silently dropped by `parseSSE`. `StepCodingKeys` has cases `type` and `name`; the `step.start` branch decodes `name` via `decodeIfPresent(String.self, forKey: .name)` from the nested `step` container.
